...
import { Github, Heart, X, Terminal, Cat, User, MapPin, Mail, MessageSquare, Maximize2, Play, Pause } from 'lucide-react';
  ...
  return (
    <div className="min-h-[200vh] flex flex-col bg-black font-mono text-white selection:bg-white selection:text-black relative overflow-hidden">
      ...
      <style>{`
        ...
        * { scrollbar-width: thin; scrollbar-color: rgba(255, 255, 255, 0.25) #000; }
      `}</style>

      {/* Header */}
      <header 
        className="fixed top-0 left-0 w-full py-3 px-6 z-40 flex items-center justify-between cursor-pointer text-sm md:text-base bg-black/60 backdrop-blur-md border-b border-white/10"
        onClick={scrollToTop}
      >
        <div className="flex items-center">
          <motion.div 
            ...
            className="pointer-events-none text-white/70 overflow-hidden whitespace-nowrap flex pr-2"
          >
        ...
        <div className="flex flex-col items-end relative">
          <div className="text-white/70 text-xs pointer-events-none flex items-center">
            <MapPin size={12} className="mr-1.5 opacity-90" strokeWidth={2} />
            <span>{hours}</span>
          ...
          <AnimatePresence>
            {showReminder && (
              ...
              >
                <div className="text-white/70 text-[9px] md:text-[10px] tracking-widest font-mono whitespace-nowrap">
                  {reminder}
                </div>
              </motion.div>
            )}
        ...
        >
          ...
          <motion.div variants={itemVariants} className="md:col-span-2 relative border border-white/30 bg-white/[0.05] hover:bg-white/[0.08] transition-colors duration-500 p-8 md:p-12 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16">
              <div>
                <h2 className="text-white/50 text-[10px] md:text-xs tracking-[0.3em] uppercase mb-6 flex items-center font-semibold">
                  <Heart size={14} className="mr-3 opacity-70" /> Likes
                </h2>
                <ul className="flex flex-col gap-4 text-white/80 text-sm md:text-base">
                  <li className="flex items-start"><span className="text-white/40 mr-4 font-bold">—</span> Cats</li>
                  <li className="flex items-start"><span className="text-white/40 mr-4 font-bold">—</span> Exploring the internet</li>
                  <li className="flex items-start"><span className="text-white/40 mr-4 font-bold">—</span> Good books</li>
                  <li className="flex items-start"><span className="text-white/40 mr-4 font-bold">—</span> Space and tech</li>
                </ul>
              </div>
              <div>
                <h2 className="text-white/50 text-[10px] md:text-xs tracking-[0.3em] uppercase mb-6 flex items-center font-semibold">
                  <X size={14} className="mr-3 opacity-70" /> Dislikes
                </h2>
                <ul className="flex flex-col gap-4 text-white/80 text-sm md:text-base">
                  <li className="flex items-start"><span className="text-white/40 mr-4 font-bold">—</span> Loud environments</li>
                  <li className="flex items-start"><span className="text-white/40 mr-4 font-bold">—</span> Pointless arguments</li>
                  <li className="flex items-start"><span className="text-white/40 mr-4 font-bold">—</span> Cluttered spaces</li>
                  <li className="flex items-start"><span className="text-white/40 mr-4 font-bold">—</span> Humans</li>
                </ul>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="md:col-span-1 relative border border-white/30 bg-white/[0.05] hover:bg-white/[0.08] transition-colors duration-500 p-8 md:p-12 rounded-lg flex flex-col">
            ...
            <p className="text-white/90 text-sm md:text-base italic font-serif flex-1 flex items-center">
          ...
          <motion.div variants={itemVariants} className="md:col-span-1 relative border border-white/30 bg-white/[0.05] hover:bg-white/[0.08] transition-colors duration-500 p-8 md:p-12 rounded-lg flex flex-col">
            ...
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-white/50 text-[10px] md:text-xs tracking-[0.3em] uppercase flex items-center font-semibold">
                <MessageSquare size={14} className="mr-3 opacity-70" /> Guestbook
              </h2>
              <button 
            ...
            <div className="flex-1 flex flex-col justify-center gap-5">
              {marks.slice(0, 3).map((mark, idx) => (
                <div key={idx} className="flex items-start gap-3 text-white/80 text-sm italic font-serif">
                  <span className="opacity-90 mt-0.5" title={mark.name}>{getEmoji(mark.name)}</span>
                  <span className="line-clamp-2">"{mark.message}"</span>
                </div>
              ))}
              {marks.length === 0 && <p className="text-white/50 text-xs italic font-serif">Quiet...</p>}
            </div>

            <button
              onClick={() => setIsModalOpen(true)}
              className="mt-6 w-full py-3 border border-white/20 hover:border-white/50 bg-white/5 hover:bg-white/10 text-white/70 hover:text-white text-xs tracking-widest uppercase transition-colors rounded font-semibold"
            >
              Open & Leave a Mark
            </button>
          </motion.div>
      ...
      <footer className="w-full flex flex-col items-center justify-end pb-4 px-4 md:px-8 relative z-10">
        <div className="w-full max-w-2xl relative border border-white/30 bg-white/[0.03] rounded-lg flex items-center justify-center min-h-[140px]">
          ...
          <AnimatePresence>
            ...
            >
              <p className="text-center italic text-white/90 text-xs md:text-sm leading-relaxed">
                <span className="font-serif text-white/50 mr-1">“</span>{quotes[quoteIndex].text}<span className="font-serif text-white/50 ml-1">”</span>
              </p>
              <span className="text-right text-white/60 text-[10px] md:text-xs tracking-tight mt-1">— {quotes[quoteIndex].author}</span>
            </motion.div>
          </AnimatePresence>
        </div>
        ...
        <div className="mt-16 flex flex-col items-center gap-6">
          <div className="flex items-center gap-8 px-8 py-3 border border-white/30 bg-white/[0.03] rounded-full">
            ...
            <a href="mailto:vanturestudios.dev@gmail.com" onClick={(e) => { e.preventDefault(); window.location.href = "mailto:vanturestudios.dev@gmail.com"; }} className="text-white/50 hover:text-white transition-all duration-300 hover:scale-110 cursor-pointer"><Mail size={18} strokeWidth={1.5} /></a>
          </div>
          <div className="flex flex-col items-center gap-3 mt-4">
            <div className="text-white/40 hover:text-white/90 transition-colors duration-500 cursor-default" title="meow."><Cat size={14} strokeWidth={1.5} /></div>
            <div className="text-white/90 text-[9px] md:text-[10px] tracking-widest uppercase font-semibold">Crafted with 🤍 by void</div>
          </div>
        </div>
      </footer>
        ...
        {isModalOpen && (
          ...
          >
            ...
            >
              ...
              <h2 className="text-white/50 text-[10px] md:text-xs tracking-[0.3em] uppercase mb-8 flex items-center font-semibold">
                <MessageSquare size={14} className="mr-3 opacity-70" /> The Guestbook
              </h2>
              ...
              {!isLeavingMark ? (
                <button
                  ...
                  className="w-full border border-dashed border-white/30 hover:border-white/60 text-white/60 hover:text-white transition-colors duration-300 py-6 rounded text-sm italic font-serif flex items-center justify-center gap-2 mb-10 bg-white/[0.02] hover:bg-white/[0.05]"
                >
                ...
                <motion.form
                  ...
                  className="flex flex-col gap-4 mb-10 border-b border-white/20 pb-8"
                  onSubmit={handleMarkSubmit}
                >
                  <input type="text" placeholder="Name (optional)" value={markName} onChange={(e) => setMarkName(e.target.value)} className="bg-black/60 border border-white/20 rounded px-4 py-2 text-sm text-white focus:outline-none focus:border-white/50 transition-colors w-full md:w-1/2" maxLength={30} />
                  <textarea placeholder="Your message (max 120 chars)" value={markText} onChange={(e) => setMarkText(e.target.value)} className="bg-black/60 border border-white/20 rounded px-4 py-3 text-sm text-white focus:outline-none focus:border-white/50 transition-colors resize-none h-24 w-full" maxLength={120} required />
                  
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mt-2">
                    <label className="flex items-center gap-3 cursor-pointer group w-fit">
                      <input type="checkbox" checked={isHuman} onChange={(e) => setIsHuman(e.target.checked)} className="hidden" />
                      <div className={`w-3.5 h-3.5 border ${isHuman ? 'bg-white/70 border-white/70' : 'border-white/40 group-hover:border-white/70'} transition-colors flex items-center justify-center rounded-sm`}>
                        {isHuman && <div className="w-1.5 h-1.5 bg-black rounded-sm" />}
                      </div>
                      <span className="text-white/60 text-[10px] tracking-[0.2em] uppercase mt-0.5 group-hover:text-white transition-colors font-semibold">I exist.</span>
                    </label>
                    <div className="flex items-center gap-4">
                      <span className="text-white/50 text-[10px] tracking-widest font-semibold">{markText.length}/120</span>
                      <button type="submit" disabled={!isHuman || !markText.trim() || isSubmitting} className="border border-white/30 text-white/80 hover:text-white hover:border-white/60 disabled:opacity-30 disabled:hover:text-white/80 disabled:hover:border-white/30 rounded px-5 py-2 text-xs tracking-widest uppercase transition-all duration-300 font-bold bg-white/5 hover:bg-white/10">
                        {isSubmitting ? '...' : 'Submit'}
                      </button>
                    </div>
                  </div>
                  {submitStatus && <p className="text-white/70 text-[10px] tracking-widest uppercase mt-2 font-semibold">{submitStatus}</p>}
                </motion.form>
              )}
              ...
              <div className="flex flex-col gap-8">
                {marks.map((mark, idx) => (
                  <div key={mark.id || idx} className="flex flex-col gap-2 border-l border-white/20 pl-4 md:pl-6 hover:border-white/50 transition-colors">
                    <p className="text-white text-sm md:text-base italic font-serif leading-relaxed">"{mark.message}"</p>
                    <div className="flex items-center gap-2 text-white/50 text-[9px] md:text-[10px] uppercase tracking-widest mt-1 font-semibold">
                ...
                {marks.length === 0 && <p className="text-white/50 text-xs italic font-serif">It is quiet here.</p>}
              </div>
